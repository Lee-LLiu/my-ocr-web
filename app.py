...
with st.expander("ℹ️ 關於我們"):
        st.write("""
        **我们致力于为零售从业者提供高效的数位化工具。
        
        我们深知超市在价格管理与盘点中的痛点，因此开发了这款基于人工智慧 OCR 技术的自动识别系统。
        透过技术手段，我们希望帮助小型门店到大型超市实现更精准、更快速的数据对齐，减少人工录入错误。
        """)
with st.expander("🔒 隐私政策 (Privacy Policy)"):
        st.write("""
        本隐私政策旨在说明我们如何处理您的数据。

        1. **数据搜集**：我们不会永久储存您上传的任何照片或 Excel 模板。所有文件仅在您当前的会话（Session）中用于识别处理，处理完成后会从伺服器缓存中清除。
        2. **第三方服务**：本工具使用百度 AI 提供 OCR 识别服务，数据传输过程经过加密。
        3. **Google 广告**：我们使用 Google AdSense 投放广告。Google 会使用 Cookie 根据您访问本网站及互联网上其他网站的情况向您投放广告。
        4. **用户权利**：您可以随时透过清除浏览器缓存或拒绝 Cookie 来停止数据搜集。
        
        如有任何隐私问题，请透过“反馈与支持”中的邮件地址联系我们。
        """)
