    ...
    if not (up_template and up_imgs and app_id):
        ...
        for img_file in up_imgs:
            ...
            if not found_products:
                st.warning(f"⚠️ {img_file.name}: 未发现匹配商品")
                continue
            ...
            for prod_name, prod_loc in found_products.items():
                ...
                if target_row:
                    ...
                    st.success(f"📸 {img_file.name} -> 识别到 【{prod_name}】: {price}")

        out_io = io.BytesIO()
        wb.save(out_io)
        st.download_button("📥 下载识别结果", data=out_io.getvalue(), file_name="multi_product_result.xlsx")
