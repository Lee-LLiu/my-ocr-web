...
with st.expander("ℹ️ 關於我們"):
        st.write("""
        **eyeonpricetag.site** 致力於為零售從業者提供高效的數位化工具。
        
        我們深知超市在價格管理與盤點中的痛點，因此開發了這款基於人工智慧 OCR 技術的自動識別系統。
        透過技術手段，我們希望幫助小型門店到大型超市實現更精準、更快速的數據對齊，減少人工錄入錯誤。
        """)
with st.expander("🔒 隱私政策 (Privacy Policy)"):
        st.write("""
        本隱私政策旨在說明我們如何處理您的數據。

        1. **數據蒐集**：我們不會永久儲存您上傳的任何照片或 Excel 模板。所有文件僅在您當前的會話（Session）中用於識別處理，處理完成後會從伺服器緩存中清除。
        2. **第三方服務**：本工具使用百度 AI 提供 OCR 識別服務，數據傳輸過程經過加密。
        3. **Google 廣告**：我們使用 Google AdSense 投放廣告。Google 會使用 Cookie 根據您訪問本網站及互聯網上其他網站的情況向您投放廣告。
        4. **用戶權利**：您可以隨時透過清除瀏覽器緩存或拒絕 Cookie 來停止數據蒐集。
        
        如有任何隱私問題，請透過「反饋與支持」中的郵件地址聯繫我們。
        """)
